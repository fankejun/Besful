...
@interface HotProdViewController ()

@end

@implementation HotProdViewController
...
//    [prodTopImageView setImage:[UIImage imageNamed:@"prod_top_head"]];
    [prodTopImageView setBackgroundColor:[UIColor clearColor]];
...
-(void)getInfo
{
    NSString * rulDetail=@"productSale";
...
-(void)downloadHeadImage:(NSString *)headUrlString
{
    ...
    NSLog(@"downloadHeadImagepostUrl==%@",postUrl);
    ASIHTTPRequest * request=[ASIHTTPRequest requestWithURL:postUrl];
    [requestArray addObject:request];
    [request setTag:DOWNLOAD_HEADIMAGE_TAG];
//    [request setUserInfo:[NSDictionary dictionaryWithObjectsAndKeys:headUrlString,@"imageUrl", nil]];
    ...
    switch (request.tag)
    {
        case DOWNLOAD_TEXT_TAG:
        {
            ...
            if ([resultStatus isEqualToString:@"true"]&&[resultMsg isEqualToString:@"ok"])
            {
                ...
                {
                    HotGoods * hotGoods = [self getAnSimpleObject:[HotGoods class] withFromDictionary:dic];
                    [self.allData_array addObject:hotGoods];
                    [self downloadImage:hotGoods.IMGSRC];
                }
                self.showData_array = self.allData_array;
        ...
        case DOWNLOAD_IMAGE_TAG:
        {
            ...
            if (image!=nil)
            {
                ...
                {
                    HotGoods * hotGoods = [self.showData_array objectAtIndex:i];
                    if ([imageUrl isEqualToString:hotGoods.IMGSRC])
                    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HotGoods * hotGoods = [self.showData_array objectAtIndex:indexPath.row];
    
    [cell.textLabel setText:hotGoods.SALENAME];//10 76 110
    [cell.textLabel setBackgroundColor:[UIColor clearColor]];
    [cell.textLabel setTextColor:[UIColor colorWithRed:10/255.0f green:76/255.0f blue:110/255.0f alpha:1.0]];
    [cell.detailTextLabel setText:hotGoods.SALEGX];//38 163 232
...
//{
//    HotGoods * hotGoods = [self.showData_array objectAtIndex:indexPath.row];
//    NSString * URLStr=hotGoods.SALEURL;
//    TaobaoViewController * taoCtl=[[TaobaoViewController alloc]init];
//    taoCtl.urlString=URLStr;
//    [self.navigationController pushViewController:taoCtl animated:YES];
//}
